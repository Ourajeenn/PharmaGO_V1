...
import { getProductStockStatus } from "@/services/stockRoutingService";
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProducts.map((product, index) => {
            const mockUserLoc = { latitude: 5.3364, longitude: -4.0267 }; // Mock Abidjan location
            const mockItemId = index === 0 ? "doli-1000" : index === 3 ? "vit-c" : "amox-500";
            const stockInfo = getProductStockStatus(mockItemId, mockUserLoc);
                    ...
                    <div className="relative">
                      ...
                      {!stockInfo.isAvailable && (
                        <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                          <Badge variant="destructive">{stockInfo.message}</Badge>
                        </div>
                      )}
                      {stockInfo.isAvailable && (
                        <div className="absolute top-3 left-3">
                          <Badge
                            className={`text-xs ${stockInfo.type === 'success' ? 'bg-emerald-500 hover:bg-emerald-600' : 'bg-amber-500 hover:bg-amber-600'}`}
                          >
                            {stockInfo.message}
                          </Badge>
                        </div>
                    ...
                    <div className="p-4 space-y-3">
                      ...
                      <div className="border-t pt-3">
                        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
                          ...
                          <span>{stockInfo.pharmacyName || product.pharmacy}</span>
                          <span>•</span>
